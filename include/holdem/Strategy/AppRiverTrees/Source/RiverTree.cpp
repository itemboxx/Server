...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------
void	clRootRiverTree::operator =  (clRoot &root)
{
	_tree = root._tree;
	_sit = root._sit;
	_gh[0] = root._gh[0];
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------
void	clRootRiverTree::CalcRiverForce(clRoot *root, int *force)
{
	ZeroMemory(force, ALL_CN_HAND * sizeof(int));
/*	tpBooleanDeck deck;
	tpCard card[5];
	for (int i = 0; i < 5; i++)
	{
			...
			if (root->_deck[i])
				for (int j = i + 1; j < CN_CARD_COL; j++)
					if (root->_deck[j])
					{
						...
						game.GameHoldemQ(hand, root->_sit.CardsTable());
						tpGame24 g0(game);
						force[nb] = g0.ToInt();
					}
	}
			...
			if (root->_deck[i])
				for (int j = i + 1; j < CN_CARD_COL; j++)
					if (root->_deck[j])
					{
						...
						int nb = hand.NbHandAll();
						int nbT, nbR = GetTurnAndRiverHandIndex(root->_cards, hand, nbT);
