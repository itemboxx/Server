...
tpDis	clStrategyH::GetDis(clSitHoldem &sit, vector <tpDisNode> &vect, clRoot &hs, int mSec)
{
	...
	{
		FindGHTurn(hs, sit);
		_turnTreeBank->GetTreeForRoot(hs, &_unfindTurn);
		//CheckTree(&hs._tree);
		t0 = GetTickCount() - t0;
		return GetDis(sit, vect, hs, mSec - t0);
	}
	...
	{
		FindGHRiver(hs, sit);
		if(!_bankRiver->BuildTreeForRoot(hs, &_unfindRiver))
			return HistCreateDis(ACT_UNDEF);
		t0 = GetTickCount() - t0;

		/*static clFileRecT <clRiverBankUnit> file;
		static bool bf=true;
		if (bf)
		{
			if (!file.IsOpen())
				file.Create("d:\\data\\temp.rbm", 0, 1000);
			if (file.CnRecord() < 1000)
			{
				clRiverBankUnit unit;
				unit._root = hs;
				unit._root._tree.ClearData();
				unit._ev[0].resize(ALL_CN_HAND);
				unit._ev[1].resize(ALL_CN_HAND);
				unit._root.CalcIndex();
				unit._root.CalcForce();
...
tpDis clStrategyH::GetDisTree(clRoot *hs, clSitHoldem &sit, vector <tpDisNode> &vect)
{
	...
	for (int i = 0; i < vn.size(); i++)
	{
		...
		if(!cls.GetWeightStrat(node->CnBranch(), w))
			return HistCreateDis(ACT_UNDEF);
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------
void	CalcRiverTreeMultiThread(clRoot &hs, int mSec)
{
	int force[ALL_CN_HAND];
	clRootRiverTree::CalcRiverForce(&hs, force);
