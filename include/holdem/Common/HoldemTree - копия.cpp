...
/*void		clHoldemTree::CalcWeightStrat(vector <tpFloat> **gh)
{
	...
	gh[ind] = &ghCur;
	int dim = gh[ind]->size();
	ErrCheck("Ќе совпадают размерности", dim, AtrTree()->CnClasters());
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------
clHoldemTree* clHoldemTree::FindSitNodeApprox(int cnAct, clSitHoldem &sit, int nextAct)
{
	if (cnAct == 0)
		return this;
	if (CnBranch() == 0)
	...
	for (; i < CnBranch(); i++)
		if (dis == Branch(i)->GetDis())
			break;
	if (i == CnBranch() && dis._act == ACT_CALL)
	{
		int i = 0;
		for (; i < CnBranch(); i++)
			if (dis._act == Branch(i)->GetDis()._act)
				break;
	}
	int cn0, mNode[MAX_CN_BRANCH];
	if (i == CnBranch() && (dis._money >= Branch(0)->GetDis()._money))
		i = 0;
	else if (i == CnBranch())
	{
		cn0 = FindAllActNode(dis._act, mNode);
		if (cn0 == 0)
			i == CnBranch();
		else if (cn0 == 1)
			i = mNode[0];
		...
		{
			int imin = 0;
			double minD = 0, money = sit.GetAct(nextAct)->_dis._money / sit.BigBlind();
			minD = Branch(mNode[0])->GetDis()._money / Branch(mNode[0])->GetSit().BigBlind();
			minD -= money;
			minD = fabs(minD);
			for (int j = 1; j < cn0; j++)
			{
				double d = Branch(mNode[j])->GetDis()._money / Branch(mNode[0])->GetSit().BigBlind();
				d -= money;
				d = fabs(d);
				if (d < minD)
				{
	...
	else if (dis._act == ACT_ALL_IN)
		i = 0;
	return (i == CnBranch()) ? NULL : Branch(i)->FindSitNodeApprox(cnAct - 1, sit, nextAct + 1);
}
//--------------------------------------------------------------------------------------------------------------------------------------------------------------
...
void	BuildTreeNLFromHistory(clHoldemTree *ptrT, clSitHoldem &sit, clSitHoldem *sitH, int nbA)
{
	...
	if (sitH != NULL)
	{
		...
		if (nbHist != -1)
		{
			...
			if (d < 0.3)
				arrDis[nbHist] = disH;  // замен€ем решение
			else
				nbHist = -1;
		}
		if (nbHist == -1) // добавл€ем решение
		{
			int i = cnDis; cnDis++;
			for (; i > 0; i--)
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------
tpDis	GetDisPushFoldRiver(clSitHoldem &sit)
{
	return HistCreateDis(ACT_UNDEF);
}
//--------------------------------------------------------------------------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------
int clHoldemTree::FindAllActNode(enAction act0,int *m)
{
	int cn0 = 0;
	if (act0 > ACT_BET && act0 < ACT_WIN)
	{
		m[cn0] = 0;
		cn0++;
	}
	for(int i = cn0 ; i < CnBranch(); i++)
		if (Branch(i)->GetDis()._act == act0 || Branch(i)->GetDis()._act == ACT_ALL_IN)
		{
			m[cn0] = i;
			cn0++;
		}
