...
//---------------------------------------------------------------------------
void    clSitHoldem::StartNextGame()
{
	_arrAct.Clear();
	int cnPl = 0;
	//создаём массив игроков для следующей сдачи
	tpSeat seatNext[CN_PLAYER];
	for (int i = 0; i < _atrHH._cnPl; i++) //_atrHH._cnPl итераций тк старая ситуация уже отнормированна по позициям
	{
		seatNext[i] = _seat[i];
		seatNext[i]._money = _pl[i]._money;
		if (seatNext[i]._money <= 0)
			seatNext[i]._inPlay = seatNext[i]._busy = false;
		else
		{
			seatNext[i]._inPlay = seatNext[i]._busy = true;
			cnPl++;
		}
	}
	//обнуляем старый массив игроков
	for (auto seat : _seat)
		seat.Clear();
	//рассаживаем оставшихся игроков не по позициям, а по местам
	for (int pos = 0; pos < _atrHH._cnPl; pos++)
	{
		if (seatNext[pos]._busy == false) continue;
		byte place = PositionToNbTable(pos);
		_seat[place] = seatNext[pos];
	}
	_atrHH._cnPl = cnPl;
	//пересаживаем игроков по позициям для новой игры
	int nb1 = NextPlayer(NbButton());
	WrNbButton(nb1);
	RestraightNbToPos();
	//создаём новый массив _pl и обуняем/инициализируем данные
	_arrAct._cnAct = 0;
	this->ExecHist(this);
	//добавляем новые акты
	if (_atrHH._ante != 0)
		for (int i = 0; i < CN_PLAYER; i++)
		{
			if (_seat[i]._busy)
			{
				...
				act._dis._money = (_seat[i]._money > _atrHH._ante) ? _atrHH._ante : _seat[i]._money;
				ExecAct(act);
			}
		}
	int SB = POS_SB;
	...
	int sb = BigBlind() / 2;
	act._dis._money = (_pl[SB]._money > sb) ? sb : _pl[SB]._money;
	if (act._dis._money > 0)
		ExecAct(act);
	act._nbSeat = BB;
	act._dis._act = ACT_POSTBB;
	act._dis._money = (_pl[BB]._money > BigBlind()) ? BigBlind() : _pl[BB]._money;
	if (act._dis._money > 0)
		ExecAct(act);
...
tpDis   clSitHoldem::MinRaise()
{
	...
	if (this->CNPlayerInTrade() == 1)
		return HistCreateDis(ACT_CALL, maxBet - PlayerBank(Indicator()));
...
//---------------------------------------------------------------------------
double	clSitHoldem::ChangeStacks(int newS)
{
	int s0 = PlMoney(0);
	double k = newS / s0;
	for (int i = 0; i < CnAct(); i++)
