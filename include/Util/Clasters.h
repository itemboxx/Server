...
int     CnClasters()                {return (int)_vect.size();}
...
{
    bool b0=false;
	unsigned dim=(unsigned)_vect.size();
...
//---------------------------------------------------------------------------
void	DivideForBin(double dist)
{
	int dim = _vect[0].size();
	int nb = 1;
	for (; nb < dim; nb++)
		if (_vect[0][0].Distance(_vect[0][nb]) > dist)
			break;
	if (nb >= dim) return;
	double *arr = new double[dim];
	int *nbArr = new int[dim];

	//double arr[1000];
	//int nbArr[1000];

	for (int i = 0; i < dim; i++)
		arr[i] = _vect[0][nb].Distance(_vect[0][i]);
	SortShellIndDef(dim, arr, nbArr);
	vector <T> vect;
	vect.resize(dim / 2);
	int i = 0; nb = dim / 2;
	for (; i < nb; i++)
		vect[i] = _vect[0][nbArr[i]];
	_vect.resize(2);
	_vect[1].resize(dim - nb);
	for (; i < dim; i++)
		_vect[1][i-nb] = _vect[0][nbArr[i]];
	_vect[0] = vect;

	delete arr;
	delete nbArr;

	FindCenterElementAndRemove(0);
	FindCenterElementAndRemove(1);
}
//---------------------------------------------------------------------------
